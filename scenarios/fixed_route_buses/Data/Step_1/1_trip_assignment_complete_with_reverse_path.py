...
class PTAnalyzer:
    ...
    def find_best_route(self, origin_xy, dest_xy, person_depart, max_walk=340):
...
if __name__ == "__main__":
    # 1. Get the absolute path to the directory where the script lives
    SCRIPT_DIR = Path(__file__).resolve().parent
    
    # 2. Go up two levels to reach the root 'buses_sumo' folder
    # Data/step_1 -> Data -> buses_sumo
    PROJECT_ROOT = SCRIPT_DIR.parent.parent 
    
    # 3. Define paths using absolute paths (safer for macOS)
    NET = str(SCRIPT_DIR / "network.net.xml")
    STOPS = str(SCRIPT_DIR / "stops.add.xml")
    BUSES = str(SCRIPT_DIR / "buses.rou.xml")
    
    # 4. Input file is in the same folder as the script
    INPUT_FILE = str(SCRIPT_DIR / "personal_planes.xlsx")

    # --- DEBUG CHECK ---
    # This will tell you exactly where the script is looking if it fails again
    for name, path in [("Network", NET), ("Stops", STOPS), ("Buses", BUSES), ("Excel", INPUT_FILE)]:
        if not os.path.exists(path):
            print(f"❌ MISSING FILE: {name} not found at: {path}")
            sys.exit(1)
    print("✅ All files found. Starting analysis...")

    # Ensure results folder exists
    (SCRIPT_DIR / "results").mkdir(exist_ok=True)

    # 5. Initialize Analyzer (Updated to use absolute paths)
    analyzer = PTAnalyzer(NET, STOPS, BUSES)
    # Ensure you are reading the correct file path
    df = pd.read_excel(INPUT_FILE)
    
    outbound_rows, return_rows, od_rows = [], [], []

    for idx, row in df.iterrows():
        # --- COLUMN MAPPING FOR NEW EXCEL FORMAT ---
        trip_id = f"t_{idx}"
        home_xy = (row['house_x'], row['house_y'])
        shop_xy = (row['destination_x'], row['destination_y'])
        
        # Mapping 'activity duration' and 'departure_time'
        shop_duration = row['activity_duration']
        departure_start = row['departure_time']
        
        # 1. GENERATE OUTBOUND (Home -> Shopping)
        out_best = analyzer.find_best_route(home_xy, shop_xy, person_depart=departure_start)
        ...
        if best_o:
            # Departure from Shop = Bus Arrival + Walk to Shop + Activity Duration
        ...
        # 3. OD DATA (Clean version for record keeping)
        od_rows.append({
            ...
            'activity_duration': shop_duration
        })


# Save files into the 'results' subfolder
    pd.DataFrame(outbound_rows).to_excel(SCRIPT_DIR / "results/Home_desination_person_info.xlsx", index=False)
    pd.DataFrame(return_rows).to_excel(SCRIPT_DIR / "results/Destination_home_person_info.xlsx", index=False)
