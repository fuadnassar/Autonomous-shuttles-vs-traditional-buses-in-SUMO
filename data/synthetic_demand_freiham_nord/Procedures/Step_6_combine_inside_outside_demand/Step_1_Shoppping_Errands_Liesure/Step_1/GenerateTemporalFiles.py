...
dist_leisure = [
    0.0044, 0.0088, 0.0044, 0.0351, 0.0351, 0.0595, 0.0595, 0.0595,
    0.0760, 0.0760, 0.0760, 0.1014, 0.1014, 0.1014, 0.0517, 0.0517, 0.0517
]
...
def get_distribution_list(col_name):
    ...
    if 'liesure' in name_lower or 'leisure' in name_lower:
        return dist_leisure
...
def calculate_duration(col_name):
    ...
    if 'liesure' in name_lower or 'leisure' in name_lower:
        duration += 6360
    if 'shopping' in name_lower or 'shopper' in name_lower:
        duration += 2100
    if 'errand' in name_lower:
        duration += 2100
...
def get_destination_info(col_name, num_rows):
    ...
    # 1. Check 'inside' + Center keywords
    if 'inside' in name_lower and 'district' in name_lower:
    ...
    elif 'inside' in name_lower and 'm1' in name_lower:
        dest_name = "Liesure M1"
        x_list = [250] * num_rows
        y_list = [520] * num_rows
        
    elif 'inside' in name_lower and 'm2' in name_lower:
        dest_name = "Liesure M2"
        x_list = [345] * num_rows
        y_list = [-100] * num_rows
        
    # 2. Check S-Bahn / U-Bahn keywords
...
def generate_column_files(input_file):
    ...
    for col_name in df.columns[1:]:
        ...
        # 7. Reorder Columns: Move 'agents' to after 'destination_y'
        # Current columns: ['Origin', 'Destination', 'agents', 'origin_x', 'origin_y', 'destination_x', 'destination_y']
        # Desired: ['Origin', 'Destination', 'origin_x', 'origin_y', 'destination_x', 'destination_y', 'agents']
...
if __name__ == "__main__":
    file_name = "All_Demand_Shoppping_Errands_Liesure.xlsx"
