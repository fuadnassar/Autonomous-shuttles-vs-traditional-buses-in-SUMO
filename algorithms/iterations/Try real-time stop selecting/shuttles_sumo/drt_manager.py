...
def run(max_dropoff_delay, max_pickup_delay, realtime_requests_limit, demand_scale):
 ...
 # ---> NEW: Ensure data is cached only once <---
    ...
    initial_buses = [v for v in traci.vehicle.getIDList() if traci.vehicle.getTypeID(v) == "arts"]
    persons.check_and_inject_persons(initial_time, pending_persons, master_database, initial_buses, bus_stops)
    # # ==========================================================
    ...
    while len(pending_persons) > 0 or len(traci.person.getIDList()) > 0:
        ...
        try:
            persons.check_and_inject_persons(current_time, pending_persons, master_database, buses, bus_stops)
...
if __name__ == "__main__":
    ...
    # Pass the terminal arguments into your run function
